...
public class AddUserJSON {
    ...
    public static interface WebServiceAddUser {

        @GET("/api/UserApi/AddUser") // deklarujemy endpoint oraz metodę
        void getData(Callback<List<AddUserJSON>> pResponse);

        @POST("/api/UserApi/AddUser") // deklarujemy endpoint, metodę oraz dane do wysłania
        void postData(@Body AddUserJSON pBody, Callback<AddUserJSON> pResponse);
    }
